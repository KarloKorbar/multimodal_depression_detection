...
def get_balanced_subset(percentage, random_state):
    ...
    balanced_subset = pd.concat([sampled_0, sampled_1]).sample(frac=1, random_state=random_state).reset_index(
        drop=True)
...
def get_face(percentage=0, random_state=42, ds_freq='50ms', rw_size='10s'):
    ...
    df_smoothed = (
        df_resampled.groupby(level='ID')
        .rolling(rw_size, on=df_resampled.index.get_level_values('TIMESTAMP'), min_periods=3)
        .mean()
        .reset_index(level=0, drop=True)
    )
